...
The core insight behind this plan is stated in its opening paragraph:
**connection point detection is a well-defined geometric computation that
Python can do precisely, while kinematic chain reasoning requires semantic
understanding that Claude does well.** Splitting the problem accordingly was
the single biggest improvement to the project.

This plan was written in response to
{doc}`Prompt 13 <building-with-claude>` and implemented in
{doc}`Prompt 14 <building-with-claude>`.
