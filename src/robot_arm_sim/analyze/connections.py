...
def detect_connection_points(
    ...
    """
    cylinders = [
        f for f in features if f.kind == "cylindrical_surface"
    ]
...
def _detect_base_connection(
    ...
    center = _find_circle_center_at_slice(
        mesh, np.array([0, 0, 1]), z_top, "below"
    )
...
def _detect_a3_4_connections(
    ...
    z_min = bounds[0][2]
    center_z = _find_circle_center_at_slice(
        mesh, np.array([0, 0, 1]), z_min, "above"
    )
    if center_z is not None:
        z_cyl = next(
            (c for c in cylinders if c.axis == [0, 0, 1]), None
        )
    ...
    x_max = bounds[1][0]
    center_x = _find_circle_center_at_slice(
        mesh, np.array([1, 0, 0]), x_max, "below"
    )
    if center_x is not None:
        x_cyl = next(
            (c for c in cylinders if c.axis == [1, 0, 0]), None
        )
...
def _detect_endpoints_along_axis(
    ...
    for end, proj_val, direction in [
        ...
        center = _find_circle_center_at_slice(
            mesh, axis, proj_val, direction
        )
        if center is not None:
            pos = center.copy()
            pos[axis_idx] = proj_val
            radius = _estimate_radius_at_slice(
                mesh, axis, proj_val, direction
            )
            points.append(ConnectionPoint(
                end=end,
            ...
            center_fb = _circle_fit_from_boundary(
                mesh, axis, proj_val, direction
            )
    ...
    for frac in [0.05, 0.10, 0.15]:
        ...
        try:
            section = mesh.section(
                plane_origin=plane_origin, plane_normal=axis
            )
        except Exception:
            continue
...
def _extract_circle_center_from_section(
    ...
    for poly in path_2d.polygons_full:
        ...
        pts_2d_h = np.array(
            [[centroid_2d[0], centroid_2d[1], 0, 1]]
        )
