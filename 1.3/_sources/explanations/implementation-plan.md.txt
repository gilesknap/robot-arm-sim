...
Reading it alongside the {doc}`building-with-claude` story, you can see which
parts survived contact with reality and which needed revision. The module
structure and pipeline stages held up well. The assembly-reasoning skill's
approach of writing raw URDF did not — that was replaced by the
{doc}`connection-point-plan` in a later phase.
...
1. Phase 1: Foundation — CLI, deps, module structure
2. Phase 2: Analyze — parser, features, renderer, YAML output
3. Phase 3: Skill — assembly reasoning skill file
4. Phase 4: Simulate core — URDF loader, FK, NiceGUI app
