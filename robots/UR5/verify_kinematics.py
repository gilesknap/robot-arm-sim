#!/usr/bin/env python3
"""Verify UR5 URDF kinematics against manufacturer DH parameters.

Usage:
    uv run python robots/UR5/verify_kinematics.py
    uv run python robots/UR5/verify_kinematics.py --json

Traces the kinematic chain at zero config (all joints = 0) and an
"arm up" test pose (J2 = -pi/2), comparing against expected positions
derived from the official UR5 DH parameters.

DH params (standard convention):
  d = [89.159, 135.85, -119.7, 109.15, 94.65, 82.3] mm
  a = [0, -425.0, -392.25, 0, 0, 0] mm
...
# UR5 expected zero-config positions (DH convention, all theta=0)
# Computed from official DH: d=[89.159, 135.85, -119.7, 109.15, 94.65, 82.3]
#                             a=[0, -425, -392.25, 0, 0, 0]
#                         alpha=[pi/2, 0, 0, pi/2, -pi/2, 0]
EXPECTED_ZERO_CONFIG = {
    "joint_1": [0.0, 0.0, 89.2],
    "joint_2": [0.0, 135.9, 89.2],
    "joint_3": [425.0, 16.2, 89.2],
    "joint_4": [817.2, 16.2, 89.2],
    "joint_5": [817.2, 109.3, 89.2],
    "joint_6": [817.2, 109.3, -5.5],
}
...
def main():
    ...
    # Arm-up test pose (J2 = -pi/2)
    print("=== Test pose: arm up (J2 = -pi/2) ===")
    import math
