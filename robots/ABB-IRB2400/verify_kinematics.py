#!/usr/bin/env python3
"""Verify ABB IRB 2400 URDF kinematics against manufacturer specs.

Usage:
    uv run python robots/ABB-IRB2400/verify_kinematics.py
    uv run python robots/ABB-IRB2400/verify_kinematics.py --json

Traces the kinematic chain at zero config (all joints = 0),
comparing against expected positions from the ROS-Industrial URDF.

Joint origins from ROS-Industrial xacro (metres):
  J1: [0, 0, 0]
  J2: [0.1, 0, 0.615]
  J3: [0, 0, 0.705]
  J4: [0.258, 0, 0.135]
...
# ABB IRB 2400 expected zero-config positions (from ROS-Industrial URDF)
# At zero config, arm extends straight up then wrist extends along X.
EXPECTED_ZERO_CONFIG = {
    "joint_1": [0.0, 0.0, 0.0],  # base (origin coincident)
    "joint_2": [100.0, 0.0, 615.0],  # shoulder
    "joint_3": [100.0, 0.0, 1320.0],  # elbow
    "joint_4": [358.0, 0.0, 1455.0],  # wrist1
    "joint_5": [855.0, 0.0, 1455.0],  # wrist2
    "joint_6": [940.0, 0.0, 1455.0],  # flange
}
...
def main():
    parser = argparse.ArgumentParser(description="Verify ABB IRB 2400 URDF kinematics")
